...
function App() {
  ...
  const [sisterContact, setSisterContact] = useState(() => {
    try {
      const saved = localStorage.getItem('sisterContact');
      return saved ? saved : '';
    } catch { return ''; }
  });
  ...
  const saveSisterContact = (contact) => {
    setSisterContact(contact);
    try { 
      localStorage.setItem('sisterContact', contact);
    } catch(e) {}
  };
    ...
    <div className="app">
      ...
      <div className="phone-wrap">
        ...
        <div className="phone-frame">
          ...
          {screen === 'report' && <WeeklyReport savedAudios={savedAudios} setScreen={setScreen} />}
          {screen === 'recording' && (
            <RecordingScreen
              setScreen={setScreen}
          ...
          {screen === 'circle' && <CircleScreen setScreen={setScreen} sisterContact={sisterContact} />}
          {screen === 'journal' && (
            <JournalScreen
              setScreen={setScreen}
              savedAudios={savedAudios}
        ...
        <div className="bottom-nav">
          ...
          <button className={`nav-item ${screen==='sister-access'?'active':''}`} onClick={()=>setScreen('sister-access')}>
            <span className="nav-icon">👭</span><span className="nav-label">Invite</span>
          </button>
