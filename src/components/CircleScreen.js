...
export default function CircleScreen({ setScreen, sisterContacts }) {
  ...
  const samplePosts = [
    { id: 's1', emoji: '🌼', tag: 'Family', tagClass: 'lc-family', color: 'rgba(244,164,147,0.2)', text: 'I don\'t know how to tell my parents I don\'t want the rishta they found. I feel so alone…', time: '2 hours ago', replies: [] },
    { id: 's2', emoji: '🌙', tag: 'Career', tagClass: 'lc-career', color: 'rgba(200,184,232,0.2)', text: 'I got the internship but my manager keeps talking over me in meetings. Is this imposter syndrome?', time: '5 hours ago', replies: [] },
    { id: 's3', emoji: '🌿', tag: 'Identity', tagClass: 'lc-identity', color: 'rgba(93,202,165,0.2)', text: 'I feel like I\'m two different people — one for home, one for college. When do I get to just be me?', time: 'Yesterday', replies: [] },
  ];
    ...
    try {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        const fetchedPosts = snapshot.docs.map(d => ({
          id: d.id, ...d.data(),
          time: new Date(d.data().createdAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }),
  ...
  return (
    <div className="screen">
      ...
      <div style={{ display: 'flex', gap: 10, width: '100%', marginBottom: 8 }}>
        ...
        {/* ✅ FRIEND CIRCLE — goes to friend-circle screen */}
        <button onClick={() => setScreen('friend-circle')} style={{
          flex: 1, background: 'rgba(200,184,232,0.12)',
          border: '1px solid rgba(200,184,232,0.3)', borderRadius: 16,
          padding: '12px 10px', display: 'flex', alignItems: 'center',
          gap: 8, cursor: 'pointer', textAlign: 'left',
        }}>
          <span style={{ fontSize: 22 }}>💛</span>
          <div>
            <div style={{ color: 'white', fontSize: 13, fontWeight: 600 }}>Friend Circle</div>
            <div style={{ color: 'rgba(255,255,255,0.45)', fontSize: 10 }}>Private live chat</div>
          </div>
        </button>

        {/* ✅ INVITE SISTER — goes to sister-access screen */}
        <button onClick={() => setScreen('sister-access')} style={{
          flex: 1, background: 'rgba(232,97,74,0.12)',
          ...
          <span style={{ fontSize: 22 }}>👭</span>
          <div>
            <div style={{ color: 'white', fontSize: 13, fontWeight: 600 }}>Invite a Sister</div>
            <div style={{ color: 'rgba(255,255,255,0.45)', fontSize: 10 }}>
              {sisterContacts?.length > 0 ? `${sisterContacts.length} invited 🌸` : 'Send email invite'}
            </div>
          </div>
        </button>
      </div>
        ...
        {allPosts.map((l, i) => (
          <div key={l.id || i} className="letter-card">
            <div className="letter-top">
              ...
              <div className={`letter-chip ${l.tagClass || tagClasses[l.tag] || 'lc-family'}`}>{l.tag}</div>
            </div>
            ...
            {l.replies && l.replies.length > 0 && (
              <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
                {l.replies.map((r, ri) => (
                  <div key={ri} style={{
                    background: 'rgba(200,184,232,0.08)', border: '1px solid rgba(200,184,232,0.2)',
                    borderRadius: 10, padding: '8px 12px', fontSize: 12, color: 'rgba(255,255,255,0.7)'
                  }}>
                    💬 {r.text}
